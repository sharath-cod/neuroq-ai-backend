...
router.get('/:id', auth, async (req, res) => {
  try {
    ...
    const [biomarkers]   = await db.execute(
      'SELECT * FROM biomarkers WHERE patient_id = ? ORDER BY recorded_on DESC LIMIT 1',
      [req.params.id]
    );
    const [genetics]     = await db.execute(
      'SELECT * FROM genetic_markers WHERE patient_id = ?',
      [req.params.id]
    );
    ...
    try {
      ...
      if (b.amyloid_beta || b.total_tau || b.hippocampal_vol || b.alpha_synuclein) {
        await conn.execute(`
          ...
          VALUES (?,?,?,?,?,?,?,?,?,?,?,?,CURDATE())`,
          [pid,
           b.amyloid_beta || null, b.total_tau || null,
           b.phospho_tau || null, b.hippocampal_vol || null,
           b.alpha_synuclein || null, b.dopamine_level || null,
      ...
      await conn.execute(`
        ...
        VALUES (?,?,?,?,?,?,?,?,?,CURDATE())`,
        [pid,
         b.apoe4 ? 1 : 0, b.family_hx ? 1 : 0, b.lrrk2 ? 1 : 0,
         b.snca ? 1 : 0, b.c9orf72 ? 1 : 0, b.sod1 ? 1 : 0,
      ...
      await conn.execute(`
        ...
        VALUES (?,?,?,?,?,?,?,?,?,?)`,
        [pid,
         b.diabetes ? 1 : 0, b.hypertension ? 1 : 0, b.smoking ? 1 : 0,
         b.depression ? 1 : 0, b.obesity ? 1 : 0, b.head_trauma ? 1 : 0,
         b.sleep_disorder ? 1 : 0, b.heart_disease ? 1 : 0, b.stroke_history ? 1 : 0]
      );
...
router.put('/:id', auth, doctorOrAdmin,
  ...
  async (req, res) => {
    try {
      ...
      await log(req.user.id, 'PATIENT_UPDATED', 'patient', req.params.id,
        `Patient ${req.params.id} updated`, req);
...
router.delete('/:id', auth, doctorOrAdmin, async (req, res) => {
  try {
    ...
    await log(req.user.id, 'PATIENT_DELETED', 'patient', req.params.id,
...
// ─────────────────────────────────────────────────────────
// POST /api/patients/:id/visit — add visit
// ─────────────────────────────────────────────────────────
  ...
  try {
    ...
    const [ins] = await db.execute(`
      ...
      [req.params.id,
       ...
       b.mmse || null, b.moca || null,
       b.updrs_motor || null, b.updrs_total || null,
       b.alsfrs || null, b.edss || null,
       b.memory_score || null, b.attention_score || null,
       b.language_score || null, req.user.id, b.notes || null]
    );
    await log(req.user.id, 'VISIT_ADDED', 'patient', req.params.id,
      `Visit added for patient ${req.params.id}`, req);
